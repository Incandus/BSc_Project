...
subroutine Grid_applyBCEdge(bcType,bcDir,guard,var,dataRow,face,&
  ...
  use Simulation_data, ONLY: sim_pAmbient, sim_rhoAmbient, sim_gamma, &
     &  sim_smallP, sim_smallX, sim_TAmbient, sim_pResev, eos_singlespeciesa
...
# include "Flash.h"
  ...
  integer :: k, sign
  real :: sim_rhoBound, sim_pBound, sim_rhoResev
  ...
  if(face==LOW) then
        ...
        sim_pBound = sim_pResev  !*3.4748  !no idea why this factor is needed
        !sim_rhoResev = ((sim_pResev/sim_TAmbient)/(8.315e7/eos_singlespeciesa))
    	!sim_rhoBound = 0.6638*((sim_pBound/sim_TAmbient)/(8.3145e7/eos_singlespeciesa)) 
    	sim_rhoBound = ((sim_pBound/sim_TAmbient)/(8.3145e7/eos_singlespeciesa))   
    	!The factor at the front makes no sense to me, but is needed to ensure the correct dens/temp ratio at start of sim
    
    	!This routine appears to only be called to return the density and pressure 
    	! (and presumably initial velocities, if important), despite the rest 
    	!of sim being dens_temp
     ...
     case(OUTFLOW)
        ...
        !!!! USER DEFINED BOUNDARY CONDITIONS BELOW
     ...
     case(USER_DEFINED)
        ...
        case(GAME_VAR)
           dataRow(1:guard)=sim_gamma
        case(DENS_VAR)
           dataRow(1:guard)=sim_rhoBound
           !dataRow(1:guard)=sim_rhoResev
           !dataRow(1:guard)=0.0
        case(PRES_VAR)
           dataRow(1:guard)=sim_pBound
           !dataRow(1:guard)=0.0
           !dataRow(1:guard)=sim_pResev*3.518
        case(TEMP_VAR)  !dummy variable
        ...
        case(VELZ_VAR)
           dataRow(1:guard)=0.0
        case(ENER_VAR)  !Energy per mass = internal energy + ke / mass of cell   !dummy variable
           kine = 0.0
           !dataRow(1:guard) = max( sim_pBound  / ((sim_gamma-1.) * sim_rhoBound) + kine, sim_smallP)
           !dataRow(1:guard) = max( sim_pResev*3.518  / ((sim_gamma-1.) * sim_rhoResev) + kine, sim_smallP)
           dataRow(1:guard) = 1.0
           
        case(EINT_VAR) !dummy variable
           !dataRow(1:guard) = max( sim_pBound  / ((sim_gamma-1.) * sim_rhoBound), sim_smallP)
           ! dataRow(1:guard) = max( sim_pResev*3.518  / ((sim_gamma-1.) * sim_rhoResev), sim_smallP)         
           dataRow(1:guard) = 1.0
        case(SPECIES_BEGIN)
