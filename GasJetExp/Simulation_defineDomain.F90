...
#include "constants.h"
  ...
  integer :: x_sweep, y_sweep

  initialDomain =.true.
  boundaries=NOT_BOUNDARY


  do x_sweep = 80 , nblks(IAXIS)
   	 initialDomain(x_sweep, 1, 1) = .false.
     boundaries(:,x_sweep,1,1)=REFLECTING
     initialDomain(x_sweep, 2, 1) = .false.
     boundaries(:,x_sweep,2,1)=REFLECTING
     initialDomain(x_sweep, 3, 1) = .false.
     boundaries(:,x_sweep,3,1)=REFLECTING
  	 !initialDomain(x_sweep, 3, 1) = .false.
     !boundaries(:,x_sweep,3,1)=REFLECTING
     if (x_sweep == 4) then
	 	!initialDomain(x_sweep, 4, 1) = .false.
     	!boundaries(:,x_sweep,4,1)=REFLECTING     
     	!initialDomain(x_sweep, 5, 1) = .false.
     	!boundaries(:,x_sweep,5,1)=REFLECTING 
     	!initialDomain(x_sweep, 6, 1) = .false.
     	!boundaries(:,x_sweep,6,1)=REFLECTING 
     	!initialDomain(x_sweep, 7, 1) = .false.
     	!boundaries(:,x_sweep,7,1)=REFLECTING 
     	!initialDomain(x_sweep, 8, 1) = .false.
     	!boundaries(:,x_sweep,8,1)=REFLECTING 
     	!initialDomain(x_sweep, 9, 1) = .false.
     	!boundaries(:,x_sweep,9,1)=REFLECTING 
     	!initialDomain(x_sweep, 10, 1) = .false.
     	!boundaries(:,x_sweep,10,1)=REFLECTING 
     	!initialDomain(x_sweep, 11, 1) = .false.
     	!boundaries(:,x_sweep,11,1)=REFLECTING 
     	!initialDomain(x_sweep, 2, 1) = .false.
     	!boundaries(:,x_sweep,2,1)=REFLECTING 
     	!initialDomain(x_sweep, 3, 1) = .false.
     	!boundaries(:,x_sweep,3,1)=REFLECTING 
